...
class DOTADataset(CustomDataset):
    ...
    def load_annotations(self, ann_folder):
        ...
        # img_file = os.listdir("/mlcv1/WorkingSpace/Personal/hienht/Dense/NWD/demo_data/images")
        # for i in range(len(img_file)):
        #     img_file[i] = img_file[i].replace(".png", ".txt")
        # ann_files = []
        # for i in range(len(img_file)):
        #     ann_files.append(os.path.join(ann_folder, img_file[i]))
        # ann_files = os.listdir(ann_folder)
        # for i in range(len(ann_files)):
            # ann_files[i] = osp.join(ann_folder, ann_files[i])
    ...
    def evaluate(self,
                 ...
                 iou_thr=0.5,
                 scale_ranges=[[0, float('inf')] ,(0, 32), (32, 96), (96, float('inf'))],
                 nproc=4,
                 train_mode=True):
        ...
        if metric == 'mAP':
            ...
            for each_iou_thr in mmcv.track_iter_progress(iou_thrs):
                mean_ap, _, recalls = eval_rbbox_map(
                    results,
                    annotations,
                    scale_ranges=scale_ranges,
                    iou_thr=each_iou_thr,
                    dataset=self.CLASSES,
                    logger=logger,
                    nproc=nproc,
                    max_det_per_image=100)
                # print(np.array(x[0]['recall']).shape)
                
                eval_results[str(each_iou_thr)] = [mean_ap, np.array(recalls[0])]
        else:
        ...
        if train_mode:
            ...
            for k, v in eval_results.items():
                mAP.append(v[0])
                recalls.append(v[1])
            mAP_result = np.mean(mAP, axis=0)
            recall_result = np.mean(recalls, axis=0).reshape(1, -1)
            print("mAP: ", mAP_result)
            print("recall: ", recall_result)

            
            result = []
            AP_all = "Average Precision (AP) @[ IoU=0.50:0.95 | area=   all | maxdets=100] = {:.3f}\n".format(mAP_result[0])
            AP_small = "Average Precision (AP) @[ IoU=0.50:0.95 | area=small  | maxdets=100] = {:.3f}\n".format(mAP_result[1])
            AP_medium = "Average Precision (AP) @[ IoU=0.50:0.95 | area=medium | maxdets=100] = {:.3f}\n".format(mAP_result[2])
            AP_large = "Average Precision (AP) @[ IoU=0.50:0.95 | area=large  | maxdets=100] = {:.3f}\n".format(mAP_result[3])
            AR_all = "Average Recall    (AR) @[ IoU=0.50:0.95 | area=   all | maxdets=100] = {:.3f}\n".format(recall_result[0][0])
            AR_small = "Average Recall    (AR) @[ IoU=0.50:0.95 | area=small  | maxdets=100] = {:.3f}\n".format(recall_result[0][1])
            AR_medium = "Average Recall    (AR) @[ IoU=0.50:0.95 | area=medium | maxdets=100] = {:.3f}\n".format(recall_result[0][2])
            AR_large = "Average Recall    (AR) @[ IoU=0.50:0.95 | area=large  | maxdets=100] = {:.3f}\n".format(recall_result[0][3])

            result.append(AP_all)
            result.append(AP_small)
            result.append(AP_medium)
            result.append(AP_large)
            result.append(AR_all)
            result.append(AR_small)
            result.append(AR_medium)
            result.append(AR_large)
            res = "".join(result)
            res = '\n' + res
            eval_res = dict()
            eval_res['result'] = res
