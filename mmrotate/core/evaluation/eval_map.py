    ...
    for k, (min_area, max_area) in enumerate(area_ranges):
        ...
        if min_area is None:
            ...
            for i in range(num_gts):
                gt_area_ignore[i] = (gt_bboxes[i, 2] * gt_bboxes[i, 3] < min_area) or (gt_bboxes[i, 2] * gt_bboxes[i, 3] > max_area)
        for i in sort_inds:
...
def get_cls_results(det_results, annotations, class_id, maxdets=100):
    ...
    #     cls_dets[i] = np.array(cls_dets[i])
    #     sort_inds = np.argsort(-cls_dets[i][:, -1])
    #     cls_dets[i] = cls_dets[i][sort_inds]
    #     cls_dets[i] = cls_dets[i][:min(len(cls_dets[i]), maxdets)]
    # cls_dets = [img_res[class_id][:min(len(img_res[class_id]), maxdets)] for img_res in det_results]
...
def average_precision(recalls, precisions, mode='area'):
    ...
    print("num_scales = recall.shape[0]: ", num_scales)
...
def eval_rbbox_map(det_results,
    ...
    for i in range(num_classes):
        ...
        cls_dets, cls_gts, cls_gts_ignore = get_cls_results(
            det_results, annotations, i, maxdets=max_det_per_image)
    ...
    return mean_ap, eval_results, recalls.reshape(1, -1)
